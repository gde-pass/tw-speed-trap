...
class GpxReplayTest {
    ...
    @Test
    fun `eastbound pass fires exactly one over-limit alert then the all-clear`() {
        val events = GpxReplay.replay(AlertEngine(listOf(camera)), trackFixes())
        assertEquals(2, events.size, "one approach: one alert, one all-clear")
        val alert = events.first() as AlertEvent.CameraAhead
        assertEquals("0241f57b8b0c", alert.camera.id)
        assertTrue(alert.distanceM in 250.0..300.0, "fixed 300 m alert radius, got ${alert.distanceM}")
        ...
        assertEquals(AlertEvent.AllClear(camera), events.last(), "passing the camera must end in an all-clear")
    }
    ...
    fun `higher tolerance suppresses the over-limit flag`() {
        ...
        val events = GpxReplay.replay(engine, trackFixes())
